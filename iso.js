const COLORS = [
  new obelisk.CubeColor().getByHorizontalColor(0x282828),
  new obelisk.CubeColor().getByHorizontalColor(0x293442),
  new obelisk.CubeColor().getByHorizontalColor(0x3B526E),
  new obelisk.CubeColor().getByHorizontalColor(0x4C6E99),
  new obelisk.CubeColor().getByHorizontalColor(0x5D8BC4)
]
...
const getSquareColor = fill => {
  switch (fill.toLowerCase()) {
    ...
    default:
      if (fill.includes('#')) {
        const cubeColors = new obelisk.CubeColor().getByHorizontalColor(Number.parseInt('0x' + fill.replace('#', ''), 16))
        return cubeColors
      }
  }
}
...
const renderStats = () => {
  const topMarkup = `
    <div class="position-absolute top-0 right-0 mt-3 mr-5">
      ...
      <div class="d-flex flex-justify-between rounded-2 border px-1 px-md-2">
        <div class="p-2">
          <span class="d-block f2 text-bold text-blue lh-condensed">${countTotal}</span>
        ...
        <div class="p-2">
          <span class="d-block f2 text-bold text-blue lh-condensed">${maxCount}</span>
      ...
      <p class="mt-1 text-right text-small">
        Average: <span class="text-bold text-blue">${averageCount}</span> <span class="text-gray-light">/ day</span>
        </p>
    </div>
  `
  ...
  const bottomMarkup = `
    <div class="position-absolute bottom-0 left-0 ml-5 mb-6">
      ...
      <div class="d-flex flex-justify-between rounded-2 border px-1 px-md-2">
        <div class="p-2">
          <span class="d-block f2 text-bold text-blue lh-condensed">${streakLongest} <span class="f4">days</span></span>
        ...
        <div class="p-2">
          <span class="d-block f2 text-bold text-blue lh-condensed">${streakCurrent} <span class="f4">days</span></span>
